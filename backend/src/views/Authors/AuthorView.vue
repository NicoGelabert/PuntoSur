<template>
  <div class="p-4 bg-white shadow rounded-lg">
    <h1 v-if="!loading" class="text-2xl font-bold">
      {{ author.id ? `Editar Autor: "${author.name}"` : 'Crear Nuevo Autor' }}
    </h1>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3">
        <div class="col-span-2 px-4 pt-5 pb-4">
          <div class="flex flex-col gap-2">
            ...
            <CustomInput class="mb-2" v-model="author.name" label="Nombre del Autor" :errors="errors['name']"/>
          </div>
          <hr class="my-4">
          <div class="flex flex-col gap-2">
          ...
          <hr class="my-4">
          <CustomInput type="checkbox" class="mb-2" v-model="author.active" :label="author.active ? 'Publicado' : 'No publicado'" :errors="errors['active']"/>
        </div>
...
function onImageChange(event) {
  ...
  if (file) {
    author.value.image = file;  // Guardamos el archivo directamente
...
onMounted(() => {
  if (route.params.id) {
    ...
    .then((response) => {
      author.value = response.data.data || {}; // Asegurar estructura de datos
      if (author.value.image) {
        author.value.image_url = author.value.image;  // Si ya existe una imagen, la asignamos para la previsualización
      }
    })
    .catch((error) => {
...
function onSubmit($event, close = false) {
  ...
  store.dispatch(action, author.value)  // Esto ahora usa el FormData que ya tienes en actions.js
  .then(response => {
    loading.value = false;
