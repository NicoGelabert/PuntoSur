@if($cookieConsentConfig['enabled'] && ! $alreadyConsentedWithCookies)
    <!-- Banner de cookies -->
    <div class="js-cookie-consent cookie-consent fixed bottom-0 inset-x-0 shadow-cookie bg-white dark:bg-gray_700 text-black dark:text-white z-10">
    ...
    <div id="cookieConsentModal" class="cookie-consent-modal hidden fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 z-50">
        <div class="modal-content bg-white dark:bg-gray_700 mx-auto p-6 rounded-xl max-w-lg mt-20 flex flex-col gap-4">
            ...
            <div class="mt-4 flex gap-4">
                <x-button class="js-cookie-consent-save btn btn-primary" onclick="saveCookiePreferences()">
                    {{ trans('cookie-consent::texts.save-preferences') }}<x-icons.send />
                </x-button>
                <x-button class="js-cookie-consent-close btn btn-secondary" onclick="hideCustomizeDialog()">
                    {{ trans('cookie-consent::texts.close') }}<x-icons.send />
                </x-button>
            </div>
        </div>
    </div>
